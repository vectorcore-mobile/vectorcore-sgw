...
func TestMMERestorationMetricsExportsSnapshot(t *testing.T) {
	reg := prometheus.NewRegistry()
	mgr := session.NewManager()
	_, _, err := mgr.Create(session.CreateParams{
	...
	handler := mmerestoration.NewHandler(mgr, mmerestoration.Config{
		Enabled:                true,
	...
	NewMMERestorationMetrics(reg, handler)

	stateMetric := findMetricWithLabels(t, reg, "sgwc_mme_restoration_state", map[string]string{
		"mme":   "10.1.1.1:2123",
		"state": "down",
	})
	...
	affectedMetric := findMetricWithLabels(t, reg, "sgwc_mme_restoration_affected_sessions", map[string]string{
		"mme": "10.1.1.1:2123",
	})
	...
	downMetric := findMetricWithLabels(t, reg, "sgwc_mme_path_down_total", map[string]string{
		"mme": "10.1.1.1:2123",
	})
